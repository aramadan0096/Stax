...
**EP7 complete** (Batch 9, branch `exec/ep7`, merged to `main` @ `07acf41`): 11/11 tasks, each strict-TDD + two-stage reviewed clean, plus a whole-branch review (ready to merge). Two new tables via numbered migrations **v19–v20** (`CURRENT_SCHEMA_VERSION` 18→20). Suite **520 passed / 0 failed / 0 xfailed**. Local-only, no cloud/API embedder, no vector DB; every AI path guards a missing embedder and returns `[]` (color search works with no model); `onnxruntime` imported lazily so the app builds/tests with it absent (all tests inject `FakeEmbedder`; real-model tests `@pytest.mark.manual`). `onnxruntime>=1.17.0` added to `pyproject.toml` (human-authorized); the ~120–170 MB CLIP model is downloaded only by `tools/download_clip_model.py` at first run, never committed. Local merge only — **not pushed** (`main` remains ahead of `origin/main`; reconciling with the remote is a separate human decision).

> **Known EP7 follow-ups** (deliberate, all inert while the embedder is `None` — activate when the model is downloaded): "Reindex library" enqueues nothing because `main.py` never assigns `settings_panel.ai_index_worker` (+ `_on_reindex_library` should refresh `self._ai_embedder`); the `ImageDropZone` is hosted hidden with no reveal control so F002 visual search has no user trigger yet (surface + `run_visual_search` exist and are tested); the real-inference path imports a not-yet-shipped `clip_tokenizer` module (needs the downloaded model, `@pytest.mark.manual` only); no startup/settings backfill enqueue of `get_elements_missing_embedding`. Minor: `get_all_embeddings` would `vstack` mixed-dim rows if ever called without a `model_id` (unreachable today); `HIST_BINS=12` is baked into stored colour blobs (unversioned).
