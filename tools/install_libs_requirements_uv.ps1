...
# 4. Export dependencies from pyproject.toml and install into .\lib\
#    (portable bundle used by bootstrap).
#
...
$exportedReqFile = Join-Path ([System.IO.Path]::GetTempPath()) ("stax-uv-export-{0}.txt" -f [guid]::NewGuid().ToString("N"))
...
if (-not $uvCmd) {
    ...
    # Refresh PATH so uv is discoverable in this session.
...
# --- Step 4: Install packages into .\lib from pyproject ---------------------
if (-not (Test-Path $pyprojectFile)) {
    Write-Err "pyproject.toml not found: $pyprojectFile"
    exit 1
}

try {
    Write-Info "Exporting dependencies from pyproject.toml ..."
    $null = & uv export `
        --project "$repoRoot" `
        ...
        --no-annotate `
        --no-emit-project `
        --output-file "$exportedReqFile"
    if ($LASTEXITCODE -ne 0) {
        Write-Err "uv export failed (exit $LASTEXITCODE)."
        exit 1
    }
    ...
    & uv pip install --python 3.9 --target "$libDir" -r "$exportedReqFile"
    if ($LASTEXITCODE -ne 0) {
    ...
    # --- Step 5: Install exported deps into .venv for interactive use -------
    # Use pip install (not sync) to avoid removing extra build tools already
    # present in the developer venv.
    Write-Info "Installing exported dependencies into .venv for interactive use..."
    & uv pip install --python "$pythonExe" -r "$exportedReqFile"
    if ($LASTEXITCODE -ne 0) {
        Write-Info "venv install encountered issues; the app will still fall back to lib/."
    } else {
        Write-Ok ".venv dependencies installed from pyproject export."
    }
}
finally {
    Remove-Item -Force -ErrorAction SilentlyContinue "$exportedReqFile"
}
